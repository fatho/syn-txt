...
use super::ast;
use super::heap;
use super::{Value, debug, span::Span};
...
impl<'a> Context<'a> {
    pub fn compile(&mut self, expr: &ast::SymExpSrc) -> heap::Gc<Value> {
        ...
        self.debug_table.insert(gced.id(), debug::DebugInfo {
    ...
    pub fn compile_exp(&mut self, expr: &ast::SymExp) -> Value {
        match expr {
            // TODO: intern symbols
            ast::SymExp::Keyword(sym) =>
                Value::Keyword(sym.0.as_ref().into()),
            ast::SymExp::Variable(sym) =>
                Value::Symbol(sym.0.as_ref().into()),
            ast::SymExp::Str(s) =>
